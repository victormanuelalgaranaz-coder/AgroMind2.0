...
// ═══════════════════════════════════════════════════════
// AUTENTICACIÓN — con fix de bucle de login
// ═══════════════════════════════════════════════════════
...
async function initAuth() {
  ...
  // ✅ FIX BUCLE: solo escuchar SIGNED_OUT para limpiar estado.
  // No manejar SIGNED_IN aquí — la navegación con location.replace
  // ya dispara initAuth con la sesión correcta guardada.
...
async function updateUserCrop(cropId, cropData) {
  try {
    const { data, error } = await supabaseClient.from('user_crops').update({
      crop_name: cropData.name, crop_emoji: cropData.emoji,
      area_hectares: cropData.area, planting_date: cropData.plantingDate,
      variety: cropData.variety || null,
      details: cropData.details || null
    }).eq('id', cropId).eq('user_id', currentUser.id).select();
    if (error) throw error;
    await loadUserCrops();
    return data;
  } catch (error) {
...
// ═══════════════════════════════════════════════════════
// PRESENCIA EN TIEMPO REAL — Productores en línea
// ═══════════════════════════════════════════════════════
...
function initPresence() {
  ...
  presenceChannel = supabaseClient.channel('agromind_online', {
  ...
  presenceChannel
    ...
    .on('presence', { event: 'join' }, () => {
      const state = presenceChannel.presenceState();
      updateOnlineCounter(Object.keys(state).length);
    })
...
function updateOnlineCounter(count) {
  const n = Math.max(1, count); // mínimo 1 (el usuario mismo)
  const el        = document.getElementById('onlineCount');
  const elMobile  = document.getElementById('onlineCountMobile');
  const counter   = document.getElementById('onlineCounter');

  if (el)       el.textContent       = n;
  if (elMobile) elMobile.textContent = n;

  // Pulso visual cuando cambia el número
  if (counter) {
...
function showLoginPage() {
  ...
  document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    if (result.success) {
      // ✅ FIX BUCLE: usar replace() en vez de reload()
      // Para cuando showLoginPage() borró el DOM original.
      // replace() hace una carga limpia y Supabase ya guardó la sesión.
