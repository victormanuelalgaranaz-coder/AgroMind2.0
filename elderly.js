...
async function initDashboard() {
  ...
  try {
    ...
    // ⬇️ INICIALIZAR CONTADOR (después de que el DOM está listo)
    initCountdown();
...
function initCountdown() {
  ...
  // Fecha objetivo: mañana a las 20:00 (hora local del dispositivo)
  // Si prefieres hora fija Bolivia (UTC-4), cambia la lógica; pero para simplicidad y cierre rápido se usa local.
  ...
  function updateCountdown() {
    ...
    if (distance <= 0) {
      ...
      if (!countdownFinished) {
        ...
        // Mostrar mensaje de finalización y ocultar el temporizador
        if (timerContainer) timerContainer.style.display = 'none';
        if (finishedMsgDiv) finishedMsgDiv.style.display = 'block';
        if (participateBtn) {
          participateBtn.disabled = true;
          participateBtn.style.opacity = '0.5';
          participateBtn.style.cursor = 'not-allowed';
          participateBtn.textContent = '🔒 Sorteo cerrado';
        }
        // Opcional: mostrar notificación adicional
        const msgDiv = document.createElement('div');
        msgDiv.className = 'countdown-finished';
        msgDiv.style.marginTop = '8px';
        msgDiv.innerHTML = '⏰ El sorteo ha finalizado. ¡Gracias por tu interés!';
        if (countdownBanner && !countdownBanner.querySelector('.final-notice')) {
          msgDiv.classList.add('final-notice');
          countdownBanner.appendChild(msgDiv);
        }
      }
      return;
    }
    ...
    // Actualizar DOM con formato de 2 dígitos
  ...
  // Si ya finalizó, no iniciar intervalo
  if (!countdownFinished) {
    if (countdownInterval) clearInterval(countdownInterval);
    countdownInterval = setInterval(updateCountdown, 1000);
  }

  // Configurar acción del botón PARTICIPAR
  if (participateBtn) {
    // Remover event listeners previos (por si se llama de nuevo)
    const newBtn = participateBtn.cloneNode(true);
    participateBtn.parentNode.replaceChild(newBtn, participateBtn);
    const finalBtn = document.getElementById('participateBtn');
    if (finalBtn) {
      finalBtn.addEventListener('click', function(e) {
        e.preventDefault();
        if (countdownFinished) {
          alert('El sorteo ya finalizó. ¡No es posible participar!');
          return;
        }
        // Aquí puedes integrar tu lógica real de participación (ej. abrir modal, enviar datos, etc.)
        alert('🎉 Gracias por participar en el sorteo. ¡Buena suerte! (Funcionalidad simulada)');
      });
    }
  }
}